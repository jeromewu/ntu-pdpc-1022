...
int main(int argc, char *argv[]){
  int i,j,n,matrix_n;
  long int datasize;
  char *cl_src = read_cl_src("median.filter.cl");

  FILE *fp = fopen(argv[1],"r");
  sscanf(argv[2],"%d",&n);
  n = pow(2,n);
  datasize = n*n*sizeof(int);

  int *array_in = (int*)malloc(datasize);
  int *array_out = (int*)malloc(datasize);
  
  fread(array_in,sizeof(int),n*n,fp);
  cl_int status;
  ...
  cl_kernel kernel;
  kernel = clCreateKernel(program,"median_filter",&status);
  ...
  fprintf(stderr,"Execution time in ms: %0.3f ms\n",(total_time/1000000.0));
